import { useMemo, useState } from 'react';

import {
  EDITABLE_COLLECTION_LOCALES,
  validateToolLogoUpload,
  type EditableCollectionKind,
} from '../../lib/adminCollections.ts';
import type { LanguageItem, SkillGroup, SkillItem, ToolItem } from '../../lib/siteData.ts';
import EditableMedia from './EditableMedia';
import { useAdminStore } from './useAdminStore';

type Props = {
  kind: EditableCollectionKind;
  title: string;
  description: string;
};

type LocaleFieldState = Record<(typeof EDITABLE_COLLECTION_LOCALES)[number], string>;
type ActiveSkillComposer = SkillGroup | null;

function createEmptyFields() {
  return { es: '', en: '', fr: '' };
}

function getSkillGroupLabel(group: SkillGroup) {
  return group === 'translation' ? 'Translation / localization' : 'SEO / content';
}

function SkillGroupSelect({
  value,
  onChange,
}: {
  value: SkillGroup;
  onChange: (group: SkillGroup) => void;
}) {
  return (
    <select
      value={value}
      onChange={(event) => onChange(event.target.value as SkillGroup)}
      className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
    >
      <option value="translation">translation — Translation / localization</option>
      <option value="seo">seo — SEO / content</option>
    </select>
  );
}

export default function EditableCollection({ kind, title, description }: Props) {
  const store = useAdminStore();
  const items = store.getEditableCollection(kind) as Array<LanguageItem | ToolItem | SkillItem>;
  const [activeComposer, setActiveComposer] = useState<EditableCollectionKind | ActiveSkillComposer | null>(null);
  const [selectedSkillGroup, setSelectedSkillGroup] = useState<SkillGroup>('translation');
  const [labelFields, setLabelFields] = useState<LocaleFieldState>(createEmptyFields);
  const [levelFields, setLevelFields] = useState<LocaleFieldState>(createEmptyFields);
  const [toolFile, setToolFile] = useState<File | null>(null);
  const [error, setError] = useState('');

  const groupedSkills = useMemo(() => ({
    translation: (items as SkillItem[])
      .map((item, index) => ({ item, index }))
      .filter(({ item }) => item.group === 'translation'),
    seo: (items as SkillItem[])
      .map((item, index) => ({ item, index }))
      .filter(({ item }) => item.group === 'seo'),
  }), [items]);

  const resetForm = () => {
    setLabelFields(createEmptyFields());
    setLevelFields(createEmptyFields());
    setToolFile(null);
    setError('');
    setActiveComposer(null);
    setSelectedSkillGroup('translation');
  };

  const openComposer = (nextComposer: EditableCollectionKind | ActiveSkillComposer, nextGroup?: SkillGroup) => {
    setActiveComposer(nextComposer);
    setSelectedSkillGroup(nextGroup ?? 'translation');
    setError('');
  };

  const addItem = async (forcedSkillGroup?: SkillGroup) => {
    if (!labelFields.es.trim() || !labelFields.en.trim() || !labelFields.fr.trim()) {
      setError('Complete ES/EN/FR values before adding this item.');
      return;
    }

    if (kind === 'languages' && (!levelFields.es.trim() || !levelFields.en.trim() || !levelFields.fr.trim())) {
      setError('Complete ES/EN/FR levels before adding this language.');
      return;
    }

    if (kind === 'tools' && !toolFile) {
      setError('Select a logo before adding this tool.');
      return;
    }

    if (kind === 'tools' && toolFile) {
      const toolLogoError = validateToolLogoUpload(toolFile);
      if (toolLogoError) {
        setError(toolLogoError);
        return;
      }
    }

    try {
      const nextIndex = store.addEditableCollectionItem(
        kind,
        kind === 'languages'
          ? { label: labelFields, level: levelFields }
          : kind === 'tools'
            ? { label: labelFields, logo: '' }
            : { group: forcedSkillGroup ?? selectedSkillGroup, label: labelFields },
      );

      if (kind === 'tools' && toolFile && nextIndex >= 0) {
        await store.setEditableToolLogo(nextIndex, toolFile);
      }

      resetForm();
    } catch (addError) {
      setError(addError instanceof Error ? addError.message : 'Could not add this item.');
    }
  };

  const renderSharedLabelFields = () => (
    <div className="grid gap-4 md:grid-cols-3">
      {EDITABLE_COLLECTION_LOCALES.map((locale) => (
        <label key={`${kind}-${locale}-label`} className="flex flex-col gap-2 text-sm text-ink">
          <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">
            Label {locale.toUpperCase()}
          </span>
          <input
            value={labelFields[locale]}
            onChange={(event) => setLabelFields((current) => ({ ...current, [locale]: event.target.value }))}
            className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
          />
        </label>
      ))}
    </div>
  );

  const renderLanguageComposer = () => (
    <div className="space-y-4 border border-black/10 bg-paper p-4 md:col-span-2 xl:col-span-3">
      {renderSharedLabelFields()}
      <div className="grid gap-4 md:grid-cols-3">
        {EDITABLE_COLLECTION_LOCALES.map((locale) => (
          <label key={`${kind}-${locale}-level`} className="flex flex-col gap-2 text-sm text-ink">
            <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">
              Level {locale.toUpperCase()}
            </span>
            <input
              value={levelFields[locale]}
              onChange={(event) => setLevelFields((current) => ({ ...current, [locale]: event.target.value }))}
              className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
            />
          </label>
        ))}
      </div>
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={() => void addItem()}
          className="inline-flex items-center gap-2 border border-black/10 bg-amaranth px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-ink transition hover:bg-ink hover:text-paper"
        >
          Save language
        </button>
        <button
          type="button"
          onClick={resetForm}
          className="inline-flex items-center gap-2 border border-black/10 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-ink"
        >
          Cancel
        </button>
        {error && <p className="text-sm text-amaranth">{error}</p>}
      </div>
    </div>
  );

  const renderToolComposer = () => (
    <div className="space-y-4 border border-black/10 bg-paper p-4 md:col-span-2 xl:col-span-3">
      {renderSharedLabelFields()}
      <label className="flex flex-col gap-2 text-sm text-ink">
        <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">Logo</span>
        <input
          type="file"
          accept="image/jpeg,image/png,image/webp,image/gif,image/svg+xml"
          onChange={(event) => setToolFile(event.target.files?.[0] ?? null)}
          className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
        />
      </label>
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={() => void addItem()}
          className="inline-flex items-center gap-2 border border-black/10 bg-amaranth px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-ink transition hover:bg-ink hover:text-paper"
        >
          Save tool
        </button>
        <button
          type="button"
          onClick={resetForm}
          className="inline-flex items-center gap-2 border border-black/10 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-ink"
        >
          Cancel
        </button>
        {error && <p className="text-sm text-amaranth">{error}</p>}
      </div>
    </div>
  );

  const renderSkillComposer = (group: SkillGroup) => (
    <div className="space-y-4 border border-black/10 bg-paper p-4">
      <label className="flex flex-col gap-2 text-sm text-ink">
        <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">Group</span>
        <SkillGroupSelect value={selectedSkillGroup} onChange={setSelectedSkillGroup} />
      </label>
      {renderSharedLabelFields()}
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={() => void addItem(group)}
          className="inline-flex items-center gap-2 border border-black/10 bg-amaranth px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-ink transition hover:bg-ink hover:text-paper"
        >
          Save skill
        </button>
        <button
          type="button"
          onClick={resetForm}
          className="inline-flex items-center gap-2 border border-black/10 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-ink"
        >
          Cancel
        </button>
        {error && <p className="text-sm text-amaranth">{error}</p>}
      </div>
    </div>
  );

  const renderMoveControls = (
    itemKind: EditableCollectionKind,
    index: number,
    disableUp: boolean,
    disableDown: boolean,
  ) => (
    <div className="flex flex-wrap gap-2">
      <button
        type="button"
        onClick={() => store.moveEditableCollectionItem(itemKind, index, -1)}
        disabled={disableUp}
        className="border border-black/10 px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-ink transition disabled:cursor-not-allowed disabled:opacity-40"
      >
        ↑ Move
      </button>
      <button
        type="button"
        onClick={() => store.moveEditableCollectionItem(itemKind, index, 1)}
        disabled={disableDown}
        className="border border-black/10 px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-ink transition disabled:cursor-not-allowed disabled:opacity-40"
      >
        ↓ Move
      </button>
      <button
        type="button"
        onClick={() => store.removeEditableCollectionItem(itemKind, index)}
        className="border border-amaranth/30 bg-paper px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-amaranth transition hover:bg-amaranth hover:text-ink"
      >
        Remove
      </button>
    </div>
  );

  return (
    <section className="space-y-5 border border-black/10 bg-white p-6 shadow-[0_18px_48px_rgba(6,4,3,0.06)]">
      <div className="space-y-2">
        <h3 className="font-heading text-3xl text-ink">{title}</h3>
        <p className="max-w-3xl text-sm leading-6 text-ink-muted">{description}</p>
      </div>

      <div className={`grid gap-5 ${kind === 'skills' ? 'xl:grid-cols-2' : 'xl:grid-cols-2'}`}>
        {kind !== 'skills' && items.map((item, index) => (
          <article
            key={item.id}
            className="space-y-4 border border-black/10 bg-paper p-5"
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="font-heading text-xl text-ink">{item.id}</p>
                <p className="text-xs uppercase tracking-[0.24em] text-ink-faint">
                  {kind.slice(0, -1)} {index + 1} of {items.length}
                </p>
              </div>

              {renderMoveControls(kind, index, index === 0, index === items.length - 1)}
            </div>

            {kind === 'tools' && (
              <EditableMedia
                src={(item as ToolItem).logo}
                mediaType="image"
                acceptKind="tool-logo"
                alt={(item as ToolItem).label[store.currentLang] ?? (item as ToolItem).label.es}
                label="🖼 Change logo"
                emptyLabel="Upload logo"
                className="aspect-[3/2] border border-black/10"
                onSelect={async (file) => {
                  await store.setEditableToolLogo(index, file);
                }}
              />
            )}

            <div className="grid gap-4 md:grid-cols-3">
              {EDITABLE_COLLECTION_LOCALES.map((locale) => (
                <label key={`${item.id}-${locale}-label`} className="flex flex-col gap-2 text-sm text-ink">
                  <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">
                    Label {locale.toUpperCase()}
                  </span>
                  <input
                    value={item.label[locale]}
                    onChange={(event) => store.updateEditableCollectionText(kind, index, 'label', locale, event.target.value)}
                    className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
                  />
                </label>
              ))}
            </div>

            {kind === 'languages' && (
              <div className="grid gap-4 md:grid-cols-3">
                {EDITABLE_COLLECTION_LOCALES.map((locale) => (
                  <label key={`${item.id}-${locale}-level`} className="flex flex-col gap-2 text-sm text-ink">
                    <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">
                      Level {locale.toUpperCase()}
                    </span>
                    <input
                      value={(item as LanguageItem).level[locale]}
                      onChange={(event) => store.updateEditableCollectionText(kind, index, 'level', locale, event.target.value)}
                      className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
                    />
                  </label>
                ))}
              </div>
            )}
          </article>
        ))}

        {kind === 'languages' && (
          <div data-collection-add="languages" className="space-y-4 border border-dashed border-black/20 bg-paper p-5 xl:col-span-2">
            <button
              type="button"
              onClick={() => openComposer('languages')}
              className="inline-flex items-center gap-2 border border-black/10 bg-ink px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-paper transition hover:bg-amaranth hover:text-ink"
            >
              Add language below the authored rows
            </button>
            {activeComposer === 'languages' && renderLanguageComposer()}
          </div>
        )}

        {kind === 'tools' && (
          <div
            data-collection-add="tools"
            className="aspect-square w-full min-w-0 border border-dashed border-black/20 bg-paper p-5 text-left transition hover:border-amaranth hover:text-amaranth"
          >
            <span className="block font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">Add tool</span>
            <span className="mt-3 block font-heading text-2xl text-ink">New logo tile</span>
            <span className="mt-2 block text-sm leading-6 text-ink-muted">Keep the same footprint as the live grid.</span>
            <button
              type="button"
              onClick={() => openComposer('tools')}
              className="mt-4 inline-flex items-center gap-2 border border-black/10 bg-ink px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-paper transition hover:bg-amaranth hover:text-ink"
            >
              Add tool
            </button>
            {activeComposer === 'tools' && <div className="mt-4">{renderToolComposer()}</div>}
          </div>
        )}
      </div>

      {kind === 'skills' && (
        <div className="grid gap-5 xl:grid-cols-2">
          <div data-skill-group="translation" className="space-y-4 border border-black/10 bg-paper p-5">
            <div className="space-y-1">
              <p className="font-heading text-2xl text-ink">{getSkillGroupLabel('translation')}</p>
              <p className="text-xs uppercase tracking-[0.24em] text-ink-faint">{groupedSkills.translation.length} skills</p>
            </div>

            <div className="space-y-4">
              {groupedSkills.translation.map(({ item, index }, position) => (
                <article key={item.id} className="space-y-4 border border-black/10 bg-white p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="font-heading text-xl text-ink">{item.id}</p>
                      <p className="text-xs uppercase tracking-[0.24em] text-ink-faint">
                        translation {position + 1} of {groupedSkills.translation.length}
                      </p>
                    </div>
                    {renderMoveControls('skills', index, position === 0, position === groupedSkills.translation.length - 1)}
                  </div>

                  <label className="flex flex-col gap-2 text-sm text-ink">
                    <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">Group</span>
                    <SkillGroupSelect value={item.group} onChange={(group) => store.updateEditableCollectionSkillGroup(index, group)} />
                  </label>

                  <div className="grid gap-4 md:grid-cols-3">
                    {EDITABLE_COLLECTION_LOCALES.map((locale) => (
                      <label key={`${item.id}-${locale}-translation-label`} className="flex flex-col gap-2 text-sm text-ink">
                        <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">
                          Label {locale.toUpperCase()}
                        </span>
                        <input
                          value={item.label[locale]}
                          onChange={(event) => store.updateEditableCollectionText('skills', index, 'label', locale, event.target.value)}
                          className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
                        />
                      </label>
                    ))}
                  </div>
                </article>
              ))}
            </div>

            <div data-collection-add="skills-translation" className="space-y-4 border border-dashed border-black/20 bg-white p-4">
              <button
                type="button"
                onClick={() => openComposer('translation', 'translation')}
                className="inline-flex items-center gap-2 border border-black/10 bg-ink px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-paper transition hover:bg-amaranth hover:text-ink"
              >
                Add translation skill
              </button>
              {activeComposer === 'translation' && renderSkillComposer('translation')}
            </div>
          </div>

          <div data-skill-group="seo" className="space-y-4 border border-black/10 bg-paper p-5">
            <div className="space-y-1">
              <p className="font-heading text-2xl text-ink">{getSkillGroupLabel('seo')}</p>
              <p className="text-xs uppercase tracking-[0.24em] text-ink-faint">{groupedSkills.seo.length} skills</p>
            </div>

            <div className="space-y-4">
              {groupedSkills.seo.map(({ item, index }, position) => (
                <article key={item.id} className="space-y-4 border border-black/10 bg-white p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="font-heading text-xl text-ink">{item.id}</p>
                      <p className="text-xs uppercase tracking-[0.24em] text-ink-faint">
                        seo {position + 1} of {groupedSkills.seo.length}
                      </p>
                    </div>
                    {renderMoveControls('skills', index, position === 0, position === groupedSkills.seo.length - 1)}
                  </div>

                  <label className="flex flex-col gap-2 text-sm text-ink">
                    <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">Group</span>
                    <SkillGroupSelect value={item.group} onChange={(group) => store.updateEditableCollectionSkillGroup(index, group)} />
                  </label>

                  <div className="grid gap-4 md:grid-cols-3">
                    {EDITABLE_COLLECTION_LOCALES.map((locale) => (
                      <label key={`${item.id}-${locale}-seo-label`} className="flex flex-col gap-2 text-sm text-ink">
                        <span className="font-body text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-ink-faint">
                          Label {locale.toUpperCase()}
                        </span>
                        <input
                          value={item.label[locale]}
                          onChange={(event) => store.updateEditableCollectionText('skills', index, 'label', locale, event.target.value)}
                          className="border border-black/10 bg-white px-3 py-2 text-sm text-ink"
                        />
                      </label>
                    ))}
                  </div>
                </article>
              ))}
            </div>

            <div data-collection-add="skills-seo" className="space-y-4 border border-dashed border-black/20 bg-white p-4">
              <button
                type="button"
                onClick={() => openComposer('seo', 'seo')}
                className="inline-flex items-center gap-2 border border-black/10 bg-ink px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-paper transition hover:bg-amaranth hover:text-ink"
              >
                Add SEO skill
              </button>
              {activeComposer === 'seo' && renderSkillComposer('seo')}
            </div>
          </div>
        </div>
      )}
    </section>
  );
}
